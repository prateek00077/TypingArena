...
return (
    ...
    <div className="w-1/2  mx-auto p-6 bg-white  rounded-xl shadow-md space-y-6">
        ...
        <div className="bg-gray-100 p-4  rounded-md">
            <p className="text-gray-900  font-medium"> {isLoggedIn ? user.username : 'Guest User'}</p>
            <p className="text-gray-600 text-sm">{isLoggedIn ? user.email : 'Not logged in'}</p>
        </div>
        {!isLoggedIn && (
        ...
        <button className="w-full px-4 py-2   border-2 rounded text-red-600  border-red-300  " onClick={logout}>
            Logout
        </button>
        <button className="w-full px-4 py-2   border-2 rounded text-red-600  border-red-300  " onClick={logout}>
            Delete Account
